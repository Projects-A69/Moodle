...
@router.get("/sections")
def get_sections(course_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_teacher_user)):
...
def add_section(course_id: UUID, payload: SectionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_teacher_user)):
    return add_section_to_course(db, payload, course_id, current_user)

@router.delete("/sections/{section_id}")
def delete_section(section_id: UUID, db: Session = Depends(get_db), current_user = Depends(get_teacher_user)):
    section = information_about_section(db, section_id)
    return delete_section_from_course(db, section, current_user)

@router.put("/sections/{section_id}")
def update_section(section: UUID, payload: SectionUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_teacher_user)):
