...
@router.get("/users")
def list_users(role: str = None, search: str = None,
               db: Session = Depends(get_db),
               current_user: UserModel = Depends(get_admin_user)):
    return admin_crud.list_all_users(db, role, search)

@router.put("/users/{user_id}/status")
def update_user_status(user_id: UUID, db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
    
    return admin_crud.update_user_active(db, user_id)

@router.get("/teachers/pending")
def get_pending_teachers(db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
    
    return admin_crud.list_pending_teachers(db)

@router.put("/teachers/{user_id}/approval")
def approve_teacher(user_id: UUID, db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
...
def update_course_visability(course_id: UUID, db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
    
    
    return admin_crud.toggle_course_visability(db, course_id)

@router.delete("/courses/{course_id}")
def delete_course(course_id: UUID, db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
    return admin_crud.delete_course(db, course_id)

@router.delete("/courses/{course_id}/students/{student_id}")
def remove_student_from_course(course_id: UUID, student_id: UUID, db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
...
def get_course_ratings(course_id: UUID, db: Session = Depends(get_db), current_user: UserModel = Depends(get_admin_user)):
