...
class TestTagsEndpoints(unittest.TestCase):
    ...
    def test_delete_tag_from_course(self, mock_delete_tag_from_course):
        mock_delete_tag_from_course.return_value = {"message": "Tag removed from course"}
        result = crud_tag.delete_tag_from_course(self.mock_db, self.course_id, self.tag_id)
        self.assertIn("message", result)
