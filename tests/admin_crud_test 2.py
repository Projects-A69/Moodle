...
class TestAdminCrud(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_admin = User(id=self.user_id, email="admin@example.com", role=Role.ADMIN, is_active=True)
        self.mock_teacher = User(id=self.user_id, email="teacher@example.com", role=Role.TEACHER, is_active=True, is_approved=False)
        self.mock_student = User(id=self.user_id, email="student@example.com", role=Role.STUDENT, is_active=True)
    ...
    def test_list_pending_teachers(self):
        teacher = Teacher(id=self.user_id, first_name="T", last_name="T", linked_in_acc="linkedin", profile_picture=None, phone_number="123")
    ...
    def test_toggle_course_visibility(self):
        course = Course(id=self.course_id, title="Course A", is_hidden=False, students=[])
    ...
    def test_remove_student_from_course(self):
        ...
        self.mock_db.query().filter().first.side_effect = [StudentCourse(), student, course]
        result = admin_crud.remove_student_from_course(self.mock_db, self.course_id, self.student_id)
        self.assertIn("message", result)

    def test_get_course_ratings(self):
        student = Student(id=self.student_id, first_name="S", last_name="T")
        student_course = StudentCourse(course_id=self.course_id, score=4.5, student_id=self.student_id)
    ...
    def test_remove_student_not_enrolled(self):
        ...
        with self.assertRaises(Exception):
            admin_crud.remove_student_from_course(self.mock_db, self.course_id, self.student_id)
